...
### Targetted vaccination strategy
### Simple solution, given knowledge of dataset
'''
for n in G.nodes():
    if G.degree(n) >= 23: # not quite right, 23 seems to be the wrong threshold
...
### Get node list sorted by degree, sorted high to low
nodes_sorted_by_degree = sorted(G.nodes(), reverse=True, key=G.degree)
from numpy.random import binomial
num_to_vaccinate = binomial(G.order(), 0.178)
for n in nodes_sorted_by_degree[0:num_to_vaccinate]:
    G.remove_edges_from(G.edges(n))
